...
## ⚡ v9.7 — AGENT BLOCKERS transparency + trend-exit on manual positions + F&O model polish

- 🚧 **AGENT BLOCKERS strip (the "agent chal raha hai par trade nahi karta" fix)** — the live diagnosis showed the #1 cause was **silent waits the user could never see**: the agent stood down for days on `wallet equity ₹0.01 < floor ₹300` with the reason buried in a server log. Now `agentStatus` computes a live **blockers array** — kill switch · LIVE preconditions · loss-cap stand-down · **equity floor (with the actual ₹ numbers)** · daily quota · cooldown countdown · futures-margin-shortage · latest soft skip ("0 candidates ≥ 75 AI score") — and the **AgentPanel renders it as a red/amber chip strip** right under the status (`🚧 AGENT BLOCKERS — entry ruka hua hai` / `🛰 AGENT WAITING`). The chip header shows the loop cadence + scan count; the status chip shows a **next-scan countdown**.
- 🛡️ **TREND-FLIP exit now guards MANUAL positions too** (user spec: "auto exit as per market trend"): when the board prints a qualifying **opposite-side** signal on a pair you hold — agent-entered OR manually-entered — the position is cut immediately (reason `TREND-FLIP (manual-held)`, Telegram pinged). **Time-exit / partial-TP stay agent-owned** — a manual swing is never force-closed by the clock, only by the trend. New `manageManualPositions` config (default **ON**) + **🛡 TREND-EXIT MANUAL** toggle in Agent Rules.
- ⚡ **Faster loop, flat API load**: the agent tick now runs every **30s** (was 60s) — trend-flip reacts twice as fast — while the in-tick wallet fetch is **throttled to ~60s** so CoinDCX API load stays flat. Stand-down-class reasons (equity floor, loss cap, kill switch, LIVE preconditions) now send **one Telegram alert per day** instead of zero.
- 🤖 **Futures-margin viability filter**: a connected wallet with **< 2 USDT free futures margin** (e.g. the account has no futures wallet — 404) can never fund a futures entry; those candidates are now filtered **before** best-pick selection so spot candidates aren't starved by a futures pick that dies at the margin gate every cycle. The `agentStart` LIVE response now carries an honest **warning when the wallet is below the equity floor** (surfaced as a toast at start, not discovered 3 days later).
- 📈 **F&O model polish (bs-model branch)**: **India put-skew** added to the synthetic chain (ATM PE ≈ +0.8 vol pts, ATM CE ≈ −0.3 — crash-insurance demand; wings still ride the smile) so PE premiums/cards price closer to real NSE quotes; the honest `bs-model` label and VIX anchor stay. **MONTHLY-ONLY indices fixed**: BANKNIFTY / FINNIFTY / MIDCPNIFTY / NIFTYNXT50 no longer guess "next Tuesday" (a contract that doesn't exist) — they use the **last Tuesday of the month** (SEBI post-rationalization reality; NIFTY weekly Tue + SENSEX weekly Thu unchanged). The F&O cards strip now actually polls at **30s** (it said "30s re-rank" but polled at 60s).
- ✅ Full matrix: **tsc 0 · 993/993 tests (52 files, +17 new guards: blockers payload incl. equity-floor text, manual trend-flip cut + OFF-respects-manual, agent-only time-exit, futures-margin filter + practice-wallet fallback, put-skew IV/price-gap invariants, monthly last-Tuesday rule + month rollover) · build 5.06s · local smoke: blockers strip live (cooldown countdown), manageManualPositions toggle accepted, skewed TOP-4 cards served, F&O paper trade open→list→close round-trip**.

---

## ⚡ v9.6 — F&O Superintelligence TOP-4 + expiry-schedule FIX + Auto-Agent 75+ AI-score gate

- 🗓️ **CRITICAL FIX — the weekly-expiry schedule was SWAPPED in v9.4** (user-verified against the live broker terminal): the code served `Nifty50 17Sep 23400 PE` while **the real Nifty weekly is 15Sep (TUESDAY)** — 17Sep ka Nifty weekly contract exists hi nahi; **SENSEX weekly = 17Sep (THURSDAY)**. v9.4's "live-verified" calendar read was wrong both ways (it set NIFTY=Thu / SENSEX=Tue). Corrected: **`WEEKLY_EXPIRY_WEEKDAY = { NIFTY: 2, SENSEX: 4 }`** (NSE=Tue, BSE=Thu — the Sept-2025 exchange regime, still current in Sept 2026). Live smoke now serves `Nifty50 15Sep 23400 PE` (dte 4) + `Sensex 17Sep 74900 PE` (dte 6). When the real NSE chain loads, its own expiryDates remain ground truth — the map only drives the honest BS-model fallback (which SENSEX always rides).
- 🧠 **F&O SIGNAL CARDS upgraded to SUPERINTELLIGENCE + TOP 4** (user spec: "top 4 cards show hone chahiye"): every index now builds **THREE ranked candidates — ATM + ITM + OTM** (one strike each side), each with a transparent **AI SCORE (0-100)** = 35% index-consensus confidence + 20% POP-at-expiry (lognormal N(d2) of strike±debit) + 15% reward:risk + 15% delta-fit (ATM ideal) + 15% grade → tiers **85+ ELITE · 75+ STRONG · 65+ ACTION · else WATCH**. The view **merges Nifty50 + Sensex and serves the TOP 4 by AI score** (30s server cache). The user's exact 4-line format stays front-and-center (STOCK NAME / Target / Entry (Buy) / Stop Loss in premium), now surrounded by the pro layer: **AI-score meter bar, trend tag, strike-bias chip (ATM/ITM/OTM), POP%, theta/day, breakeven, expected index move, per-lot cost/risk/reward, 3-tier exit plan (T1 50% book→BE-lock · T2 40% · hard stop · 15:10/14:30 time-exit), and a Hinglish machine note**. `tradeable` now mirrors the auto-agent bar: **STRONG/ACTION grade YA 75+ AI score**.
- 🤖 **AUTO-AGENT: the "75+ AI Score pe bhi auto trade nahi ho raha" root cause fixed** — the agent's entry gate demanded **STRONG grade + confidence ≥ 80% + agreement ≥ 75%** (stricter than anything the user sees on the board), so signals with AI score 75–84 never fired. New dual gate: **75+ AI score (`superIntel.aiScore`) qualifies on its own**; the old STRONG-committee bar still works in parallel (jo bhi pehle qualify kare). New `minAiScore` knob (default 75, UI slider in Agent Rules), picks strips now show the 🧠 AI score, and the candidates rank **AI-score first**. LIVE mode still rides the full execution gauntlet (fresh STRONG re-check server-side) — paper/notify relax exactly as manual clicks do.
- 🔄 **TREND-FLIP auto-exit (user spec: "auto exit as per market trend")** — every agent cycle now scans the boards **before** the quota/cooldown gates and cuts a held position the moment the board prints a **qualifying OPPOSITE-side signal** on that pair (reason `TREND-FLIP`, Telegram pinged, total P&L incl. booked T1/T2 legs). A cooldown is stamped on the flip so the other side can only re-enter after `cooldownMin` — no whipsaw churn. Time-exit and the 3-tier partial-TP machinery are unchanged.
- 🖥️ **Vercel/static-host deployment hardened**: `getProxyBase()`'s hardcoded fallback for `.vercel.app`/`.github.io`/`.netlify.app` now points at the **CURRENT backend** (`smartai-e954.onrender.com` — the old `smartback-iyuq` URL was a dead service, silently breaking every Vercel deploy that didn't set `VITE_API_PROXY`); login/logout now resolve the backend via `getProxyBase()` like the rest of the app (was raw env-only). On Vercel set `VITE_API_PROXY=https://smartai-e954.onrender.com` as a **Config** (non-secret) variable — it's a public URL, Vite inlines it in the browser bundle by design; and add the Vercel origin to the backend's `ALLOWED_ORIGINS` (CORS is fail-closed in production).
- ✅ Full matrix: **tsc 0 · 976/976 tests (52 files, +7 new guards: NIFTY=Tuesday/SENSEX=Thursday schedule, per-candidate level-ordering invariant, AI-score/tier/POP/strike-bias/ranking/exit-plan shape, agent 75+ AI-score gate fires below-80 confidence, below-bar 70 score blocks, trend-flip cut + cooldown stamp, minAiScore clamp) · build 4.96s · live smoke: NIFTY 15Sep (dte 4) + SENSEX 17Sep (dte 6) desks, TOP-4 cards served with AI scores + methodology string, agent config accepts minAiScore=75**.

---

## ⚡ v9.5 — F&O option paper trading + futures gate fixes

- 🎯 **OPTION PAPER TRADING (F&O)**: the Paper Desk now accepts **OPTION trades** (`assetKind=OPTION`) — premium-LONG positions with **lot-multiplied P&L**, BS re-priced every watcher tick from live index spot (Yahoo ^NSEI/^BSESN), full SL/T1/T2/trail/15:10 discipline; the option identity survives device-mirror restore. **OptionSignalCard got a one-click 🧪 PAPER TRADE (1 lot) button**; PaperTradePanel shows contract labels + an F&O badge + lot-adjusted P&L.
- 🔀 **Futures execution-gate fixes**: `evaluateExecutionGate` accepts **BUY/SELL as LONG/SHORT aliases** (fixed futures paper trades dying on "signal side is LONG, requested BUY"); futures paper mode relaxes exchange min-qty with an honest note (LIVE keeps the hard reject); option underlyings injected into the SSE watch set; index symbols ride Yahoo quotes (Groww CASH/NIFTY ltp is garbage — observed 19425 vs real 23398).
- ✅ 970/970 tests (50 files), tsc clean, build 5.1s.
