...
## v13.5 — COINDCX MIRROR AGENT + FULL-SITE RECHECK + CLEANUP (2026-09-26)

**User spec: "full site cleanup, ya CoinDCX mirror agent dono bhi karo aur full site code recheck karo"** — all three delivered in one build. Two parallel deep audits (frontend import-graph + server module-graph, every finding verified by hand before touching code) → 9 real bugs fixed, 1 honest-config fix, ~1,900 lines of dead code deleted with zero functional loss.

### THE FEATURE — CoinDCX Mirror Agent (`server/ai/cryptoAgent.js` + `CryptoAgentPanel.tsx`)
The desk chat agent could see the trading WALLET (`get_wallet`) and the app's journal positions (`get_open_positions`) — but NOT the user's REAL exchange account. Two new tools mirror it straight from api.coindcx.com:
- **`get_exchange_holdings`** — every spot coin holding: qty, avg cost basis (trade-ledger or manual), live INR value, invested, P&L ₹/%, 24h change, total portfolio value — the Portfolio tab's asset table (fetchCoinDcxAssets, dust-filtered, never invents a value), sorted value-desc; no-basis coins show honest `null` P&L, not zero.
- **`get_exchange_orders`** — what is actually RESTING on the exchange book (open + partially_filled via listExchangeOrders): pair, side, type, price, filled/remaining qty, age — the previously UI-unrouted `/api/ai/orders` surface, finally reachable from chat.
- System prompt now routes REAL-account questions ("mere paas kya coins hain", "koi pending order hai?") to the mirror tools and forces the agent to say WHICH mirror it showed (exchange vs journal — never confuse the two). New 🪙 **Account Mirror** quick-chip on the CoinDCX panel.
- Honest degrade: not connected → clear connect-note; API failure → the error, not silence.

### Full-site recheck — the 10 fixes (every one audit-verified, then reproduced in source)
1. **PortfolioHealthMonitor digest date** — the toLocaleString→toISOString dance re-shifted the "today" key by the BROWSER offset (East-Asian users got the 8 AM IST digest every ~2 days); key AND hour now come from one en-CA/Asia-Kolkata formatter (the locked v10.13 pattern).
2. **DailyPLTracker "today" row** — the live entry used raw UTC ISO dates while the recorder bucketed by browser-local key (duplicate/mislabeled today row 00:00–05:30 IST + month-boundary misbucketing); `todayKey()` is now exported and IST-based, used by both sides.
3. **`/api/ai/bandwidth` ignored env caps** — the route called `bandwidthView()` with no env (hardcoded 5GB/70% fallback) while the Telegram alert used the real env → panel vs alert disagreed; route now passes `BANDWIDTH_MONTHLY_CAP_GB`/`BANDWIDTH_ALERT_PCT` through.
4. **cxRtStream repeating unhandledRejection** — `_pollOnce`'s try/finally had no catch: a throw in the post-poll recovery steps rejected the inflight promise the interval discards → unhandledRejection every 2–10s AND the WS-recovery steps got skipped; now settles once per beat with a bounded warn.
5. **render.yaml cron `meta-ensemble-retrain` never built** — `rootDir: ml-service` + `pip install -r ml-service/requirements.txt` pointed at a nonexistent nested path; fixed to `requirements.txt`.
6. **AI_ENABLE_META_ENSEMBLE honest-config fix** — the flag was ON but the Node consumer (aggregateVotesWithMeta) is test-only, no FastAPI service is deployed, and the trained artifacts are gitignored: signals NEVER touched the meta-learner. Flag OFF in render.yaml + .env.example with the exact re-enable recipe (dead-flag-off > fake-feature-on; the in-process weighted ensemble is what production ran either way).
7. **NewsSentimentFeed engine label** — footer said Groq llama-3.3-70b while the request sends openai/gpt-oss-120b.
8. **usePrefetch fundamentals prefetch was unreachable** — the third `currentSymbol` param was never passed (no app-level selected-symbol state exists); dead block + param deleted instead of fake-wired.
9. **Build-time PROXY_BASE in 9 call sites** — NeuralChat, LiveCandleChart, NewsSentimentFeed, fundamentalsApi, entryPriceEngine, inflationEngine, earningsCalendar, sentimentEngine, superintelligenceEngine baked `import.meta.env.VITE_API_PROXY` at module scope, ignoring the runtime `WEALTH_AI_BACKEND_URL` override; all now resolve `getProxyBase()` per call (the v10.13 live-base fix finally site-complete).
10. **Raw fetch() → apiFetch** (LiveCandleChart chart feed, fundamentalsApi) — session token rides along; harmless on today's public routes, correct if auth ever tightens.

### Full site cleanup — ~1,900 dead lines removed, zero functional loss
- **Dead systems:** Toast notification system (provider wrapped the whole app; ZERO useToast/addToast call sites — every error path uses alert()) — file + wrapper deleted. Client-side Telegram report-generator subsystem (generateNeuralInsiderResponse + its 18 orphaned generate*Report helpers — 522 lines, reachable from nothing). ShortcutsHelp modal + useKeyboardShortcut (never rendered).
- **db.ts v2 upgrade:** 4 of 6 IndexedDB stores were dead-from-day-one — transactions (ledger uses localStorage), priceHistory (never written), offlineQueue (never drained), portfolioSnapshots (**write-only**: App.tsx committed a row per minute, nothing ever read it). DB_VERSION 2 deletes the stores from existing browsers (reclaims the junk) and the module keeps only the live surface (chat history + user prefs).
- **Dead exports (30):** api.ts isCloudSyncConfigured/formatMarketIntelligenceForAI · assetPnl livePriceFor · backtestEngine formatBacktestForTelegram · cache cached/preloadCache · constants TAX_PAIRS · dipEngine enhanceDipSignalWithML/enhanceAllDipsWithML (the unwired dip-ML subsystem) · earningsCalendar predictEarningsWithAI/formatEarningsForTelegram · fundamentalsApi clearFundamentalsCache · portfolioAnalytics assetAllocationBreakdown/ASSET_CLASS_* · sentimentEngine formatSentimentForTelegram · smartMoney formatFIIDIIMessage · telegram generateNeuralInsiderResponse · wealthEngine adjustForInflation (compareSIPStepUps KEPT — it has a live internal caller the export-scan missed) · server: capArray, fetchCryptoSnapshot, dhanPositions/dhanOrderStatus, fetchFuturesLtpMap, getOptionsContext, futBookRow/futBookLastUpdateAt, cryptoStreamEnabled, usStreamEnabled.
- **Stale files:** docs/AUDIT_FIX_REPORT_v1801.md, UPGRADE_REPORT.md, UPGRADE_REPORT_v18.md (one-time historical reports — CHANGES.md is the living log) · scripts/smoke_v113.mjs + smoke_v120.mjs (superseded by v128/v131/v133) · server/apps-script/oldcode.gs (stale backup; Code.gs is live).
- **Verified-live, kept:** ml-service (deployable via its Dockerfile — see fix #6 for the honest gap), telegram-bot (forked by index.js, Dockerfile-copied), backtest_ab_v2.mjs (documented A/B validation), requestQueue/tvWebsocket test hooks, the owner-only ops routes (/api/ai/orders etc. — now ALSO reachable via the mirror agent).

**Tests:** cryptoAgent suite extended 28→35 (registry pins the 20-tool list, REAL-exchange semantics on the mirror tools, holdings mapping/totals/sort/no-basis-null, orders mapping/filled-math/age-desc/empty-note, honest not-connected + failure paths). Full suite **2494/2494 GREEN** (+8). tsc clean · build 5.3s · check-api-routes PASS · node --check all touched files.
