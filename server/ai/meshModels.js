...
const abstain = (reasons) => vote(0, 0, reasons);
...
export function instFlowProVote(ctx) {
  ...
  if (mkt !== 'GLOBALFUTURES') {
    return abstain(['InstFlowPro abstains — Quiver congressional/insider alt-data covers US tickers (global desk only)']);
  }
...
export function techConsensusVote(ctx) {
  ...
  if (mkt !== 'GLOBALFUTURES' && mkt !== 'INDIA') {
    return abstain(['TechConsensus abstains — the independent vendor read serves the equity desks (global/India)']);
  }
...
export function fundaProPlusVote(ctx) {
  ...
  if (mkt !== 'INDIA' && mkt !== 'GLOBALFUTURES') {
    return abstain(['FundaProPlus abstains — fundamentals serve the equity desks (India swing / global)']);
  }
...
export function cryptoOnChainProVote(ctx) {
  ...
  if (mkt !== 'CRYPTO' && mkt !== 'FUTURES') {
    return abstain(['CryptoOnChainPro abstains — on-chain/dev context serves the crypto desks']);
  }
